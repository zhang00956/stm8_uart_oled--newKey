...
//#if BANBEN2
//    GPIO_Init(GPIOB, GPIO_Pin_7,GPIO_Mode_Out_PP_High_Slow ); //辅助 1：0 灭  
...
//#else
//    GPIO_Init(GPIOB, GPIO_Pin_7, GPIO_Mode_Out_PP_Low_Slow); //辅助 0：1 灭
//    GPIO_Init(GPIOB, GPIO_Pin_6, GPIO_Mode_Out_PP_High_Slow );
//
//#endif    
#if FAN
    GPIO_Init(GPIOB, GPIO_Pin_7, GPIO_Mode_Out_PP_Low_Slow); //辅助 1：0 灭
    GPIO_Init(GPIOB, GPIO_Pin_6, GPIO_Mode_Out_PP_High_Slow );
#else
...
//             break;
        case KeyPassLong:                 //消抖及长按确认
            if (key_press == level) {
                keyTimeCnt++;
                if( keyTimeCnt > 10 ) {  //如果长按时间到，返回值为长按状态
                    keyTimeCnt = 0;
